...
import { chatApi, billApi } from '../api'
  ...
  const load = async (d) => {
    ...
    try {
      const [repRes, billRes] = await Promise.all([
        chatApi.getReports(d),
        billApi.getStats()
      ])
      setReport(repRes.data)
      setBillStats(billRes.data)
    } catch(e) {
  ...
  return (
    <div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(5,1fr)', gap:12, marginBottom:16 }}>
        {[
          { label:'Daily Profit', value:`₹${(billStats?.dailyProfit || 0).toLocaleString()}`, color:'var(--purple)', bg:'var(--purple-xl)' },
          { label:'Weekly Revenue', value:`₹${(billStats?.weeklyRevenue || 0).toLocaleString()}`, color:'var(--teal)', bg:'var(--teal-xl)' },
          ...
          { label:'Top Prods (#)', value:report.topItems?.length || 0, color:'var(--amber)', bg:'var(--amber-xl)' },
        ].map(s => (
          <div key={s.label} style={{ background:'white', border:'1px solid var(--border)', borderRadius:12, padding:16, boxShadow:'0 1px 4px rgba(0,0,0,0.06)' }}>
             <div style={{ fontSize:10, color:'var(--muted)', textTransform:'uppercase', letterSpacing:'0.06em', fontWeight:500 }}>{s.label}</div>
             <div style={{ fontFamily:"'Syne',sans-serif", fontSize:18, fontWeight:700, color:s.color, marginTop:4 }}>{s.value}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ marginBottom:12 }}>
        <div style={{ fontFamily:"'Syne',sans-serif", fontSize:20, fontWeight:700, color:'var(--text)', marginBottom:12 }}>📊 Yearly Performance Analysis</div>
        <YearlyComparison />
      </div>

      <div style={{ background:'white', border:'1px solid var(--border)', borderRadius:12, padding:16, boxShadow:'0 1px 4px rgba(0,0,0,0.06)' }}>
        <div style={{ fontSize:14, fontWeight:600, marginBottom:12, color:'var(--text)' }}>Top Selling Items</div>
        {!report.topItems?.length ? (
          <div style={{ color:'var(--muted)', fontSize:13, textAlign:'center', padding:'24px 0', background:'var(--bg3)', borderRadius:8 }}>
            No sales recorded yet. Update stock via AI Chat!
          </div>
        ) : (
          report.topItems.map((item, i) => (
            <div key={item.name} style={{ display:'flex', alignItems:'center', gap:14, padding:'12px 0', borderBottom: i < report.topItems.length - 1 ? '1px solid var(--border)' : 'none' }}>
              <div style={{ width:28, height:28, borderRadius:8, background:'var(--purple-xl)', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:"'Syne',sans-serif", fontSize:13, fontWeight:700, color:'var(--purple)', flexShrink:0 }}>
                #{i + 1}
              </div>
              <div style={{ flex:1 }}>
